...
double ConfusionMatrix::f_score() const {
    return (double) (2 * precision() * detection_rate()) / (double) (precision() + detection_rate());
}
...
double ConfusionMatrix::error_rate() const {
    return (double) (GetFP() +  GetFN()) / (double) (GetTP() + GetTN() + GetFP() + GetFN());
}
